...
{
    public class login_errorModel : PageModel
    {
        ...
        RunDB runDB;
        public String pageName = "/start/login_error", contextRoot = "CSBCA";
            ...
            if (userCode != null && userCode.Length > 0)
            {
                    ...
                    try
                    {
                        if (lang.Equals("en"))
                        {
                            if (userGroup.Equals("Admin"))
                                return RedirectToPage("/admin/admin"); //response.Redirect("admin/admin");
                            else if (userGroup.Equals("Supervisor"))
                                return RedirectToPage("/user/user");
                            else if (userGroup.Equals("Normal"))
                                return RedirectToPage("/user/message_rew");
                        }
                        else
                        {
                            if (userGroup.Equals("Admin"))
                                return RedirectToPage("/admin/yonetim"); //response.Redirect("admin/yonetim");
                            else if (userGroup.Equals("Supervisor"))
                                return RedirectToPage("/user/kullanici");
                            else if (userGroup.Equals("Normal"))
                                return RedirectToPage("/user/mesaj_bak");
                        }
                    }
                    catch (Exception ex)
                    {
                ...
                else
                {
                    try
                    {
                        ...
                        if (lang.Equals("en"))
                            return RedirectToPage("/start/login_error");
                        else
                            return RedirectToPage("/start/giris_hatasi");
                    }
                    catch (Exception ex)
                    {
                        Debug.WriteLine(ex.Message);
