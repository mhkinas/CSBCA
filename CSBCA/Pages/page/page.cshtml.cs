...
namespace CSBCA.Pages.page
{
    ...
    {
        public PageParams p = null;
        PageProcess process = null;
        ...
        private void initModel()
        {
            ...
            pageName = p.pageName; 
            excelCols = p.excelCols; pageTitle = p.pageTitle;
        }
        public void OnGet()
        {
            try
            {
                process = new PageProcess();
        ...
        public IActionResult OnPost() //JsonResult
        {
            ...
            try
            {
                if (process == null) process = new PageProcess();
                ...
                if (p.action.Equals("download"))
                {
                    if (p.isDebug) Console.WriteLine("Download is begun!");

                    buffer = process.doDownload(); //fileStream

                    if (buffer != null)
                    {
                        return File(buffer, "application/octet-stream", p.fieldNames[1]);//fileSResult;
                    }
                    else
                    {
                        if (p.isDebug) Console.WriteLine("File buffer is null!");
                        return null;
                    }
                }
                else
                ...
                // For file upload
                if (p.action.Equals("insert") || p.action.Equals("update"))
                {
                    if (p.isDebug) Debug.WriteLine("p.action : is right!");

                    if (!process.doUpload())
                        Debug.WriteLine("Upload yapılamadı!");
                }
                p = process.p;
                messages = process.messages;
                messageString = process.messageString;
