...
function getPage(page) 
{
	...
	if(window.totRow >= page * window.lineCount)
	{
		k = page * window.lineCount;
		n = k - window.lineCount ;
	}
	else if(window.totRow >= (page-1) * window.lineCount)
	{
		k = window.totRow;
		n = (page-1) * window.lineCount;
	}
	else
	{
		return;
			...
			{				
				if(j == 0)
				{
					if(window.isModal)
						...
						tableStr += "<tr><td>"+order+"</td><td onclick = \"getUpdateForm("+window.dataList[i][fieldNames[j]]+
									")\"";
				}	
				else
					tableStr += "<td";
...
// For sending file info to imputs
...
// For sending file name to imput
